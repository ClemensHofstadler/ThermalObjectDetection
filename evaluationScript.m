...
function eval = ourEvaluate(net, X_test, Y_test, gridSize, inputSize, candidate_confidence, iou_threshold, visualize)
    ...
    for i = 1:size(X_test,1)
       detection = bbsFusion(prediction(net, X_test{i}, gridSize, inputSize), candidate_confidence);
       bboxes{i} = detection.bbs;
       scores{i} = detection.confidences;
    end
...
function [X,Y] = loadEvaluationData(data_root_folder, inputSize, gridSize)
    ...
    %first flatten the labels in data_test struct
    Y = cell(length(X_test),1);
    idx = 1;
    for i = 1:length(data_test)
        current_folder = data_test(i).seq;
        Y_from_folder = cell(length(current_folder),1);
        for j = 1:length(current_folder)
        	current_seq = current_folder(j);
            mid = ceil(size(current_seq.image,2)/2);
            Y_from_folder{j} = polyToBB(current_seq.poly{mid});
        end
